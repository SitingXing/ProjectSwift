...
import OverviewLabel from "../components/ProjectDetail/Overview/OverviewLabel";
...
function DashboardScreen({ navigation }) {
  ...
  const formatUserTasks = () => {
    const listWithProjectName = [...currentUserTasks].map((item) => {
      const projectWithName = projects.find(
        (project) => project.key === item.projectId
      );
      return {
        ...item,
  ...
  useEffect(() => {
    ...
    dispatch(
      subscribeToCurrentUserTasks(currentUser.projectsList, currentUser.key)
    );
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={[styles.date, { fontFamily: "Poppins_600SemiBold" }]}>
          {date}
        </Text>
      ...
      <ScrollView style={styles.contentContainer}>
        ...
        <View>
          ...
          />
          <View
            style={taskShow ? styles.myTasksContainer : { display: "none" }}
          >
