...
function TaskDetailScreen({ route, navigation }) {
  ...
  const commentsList = [...currentProjectComments].filter(
    (ele) => ele.taskId === task.key
  );
  const comments = commentsList.length === 0 ? [] : commentsList[0].comments;
    ...
    <View>
      ...
      <ScrollView style={styles.contentContainer}>
        <View style={styles.content}>
          ...
          <Text style={[styles.info, { fontFamily: "Poppins_400Regular" }]}>
            {task.taskName}
          </Text>
        </View>
        <View style={styles.content}>
        ...
        <View style={styles.content}>
          <View style={styles.commentLabel}>
            ...
            >
              <Text
                style={[styles.comment, { fontFamily: "Poppins_500Medium" }]}
              >
                Comment
              </Text>
            </TouchableOpacity>
          ...
          <ScrollView style={styles.commentsList}>
            {comments.map((comment, index) => {
              return <TaskItemComment key={index} comment={comment} />;
            })}
          </ScrollView>
        </View>
      </ScrollView>
      ...
      >
        ...
        <View style={styles.atContainer}>
          {atMembers.map((mem, index) => {
            return (
              <TouchableOpacity key={index} onPress={() => cancelSelect(mem)}>
                <Image source={{ uri: mem.profile }} style={styles.profile} />
              </TouchableOpacity>
            );
          })}
          <TouchableOpacity onPress={() => setMembersShow(true)}>
            <Icon
              type="ant-design"
              name="pluscircle"
              size={35}
        ...
        <View style={styles.overlayBtn}>
          <TouchableOpacity style={styles.editBtn} onPress={handleComment}>
            <Text style={[styles.edit, { fontFamily: "Poppins_600SemiBold" }]}>
              Comment
            </Text>
          </TouchableOpacity>
          ...
          >
            <Text
              style={[styles.delete, { fontFamily: "Poppins_600SemiBold" }]}
            >
              Cancel
            </Text>
          </TouchableOpacity>
        ...
        {currentProject.members.map((mem, index) => {
          return (
            ...
            >
              <Image source={{ uri: mem.profile }} style={styles.profile} />
              <Text
                style={[styles.userName, { fontFamily: "Poppins_400Regular" }]}
              >
                {mem.userName}
              </Text>
            </TouchableOpacity>
